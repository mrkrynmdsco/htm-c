...
void test_bitarray_Creation(void) {
    ...
    uint16 nbits = 4096;
    Array* dummy_array;
    Array_Create_fake.return_val = dummy_array;
    // then
    BitArray* arr = NULL;
    arr = BitArray_Create(nbits);
    TEST_ASSERT_NOT_NULL(arr);
    TEST_ASSERT_EQUAL(arr->darray, dummy_array);
}

void test_bitarray_Get_Dense_data_pointer(void) {
    // when
    uint16 nbits = 16;
    Array* dummy_array;
    void* dp;
    Array_Create_fake.return_val = dummy_array;
    Array_GetDataPtr_fake.return_val = dp;
    // then
    BitArray* arr = NULL;
    arr = BitArray_Create(nbits);
    uint8* vp = BitArray_GetDense(arr);
    TEST_ASSERT_EQUAL(vp, (uint8*)(dp));
}
